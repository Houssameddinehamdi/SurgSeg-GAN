...
# Define the class colors to keep
keep_colors = [
...
# Output folder
output_base_folder = r"D:\Dataset_parts\Datasplit\Labels_for_7classes"
os.makedirs(output_base_folder, exist_ok=True)

def process_image(input_path, output_path, keep_colors):
    # Open the image
    ...
    for y in range(img.height):
        for x in range(img.width):
            if pixels[x, y] in keep_colors:
                processed_pixels[x, y] = pixels[x, y]  # Keep the original color
            else:
...
for folder_type, folder_path in input_folders.items():
    ...
    for filename in os.listdir(folder_path):
        if filename.endswith(".png"):  # Process only PNG images
            ...
            # Process the image
            process_image(input_path, output_path, keep_colors)
